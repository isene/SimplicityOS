...
"status-line" [ draw-mode draw-x draw-y move-cursor ] define
...
"handle-insert" [ dup key-escape = if drop exit-insert else dup 13 = if drop editor-enter else dup 8 = if drop editor-backspace else dup key-delete = if drop editor-delete else dup key-left = if drop editor-left else dup key-right = if drop editor-right else dup key-up = if drop editor-up else dup key-down = if drop editor-down else dup 32 >= over 126 <= and if insert-char else drop then then then then then then then then then ] define
